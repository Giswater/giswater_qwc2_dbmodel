...
CREATE OR REPLACE FUNCTION "SCHEMA_NAME"."gw_fct_setmincut_start"(p_mincut_id int4, p_device int4, p_user_name text) RETURNS pg_catalog.json AS $BODY$
DECLARE
...
--  Update values
    UPDATE om_mincut SET mincut_state=1, exec_start=now(), exec_user=current_user WHERE id=p_mincut_id;

-- Get mincut class and mincut feature
    SELECT mincut_class INTO v_mincut_class FROM om_mincut WHERE id=p_mincut_id;
